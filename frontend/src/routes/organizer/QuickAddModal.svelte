<script>
    ...
    const PRIORITIES = [
        ...
        { value: 'low',    label: 'Low',    active: 'bg-slate-700 text-slate-400', idle: 'bg-slate-700 text-slate-500 hover:bg-slate-600' },
    ];
    ...
    function submit(startNow) {
        ...
        const dur = form.duration_min ? parseInt(form.duration_min) : 60;
        dispatch('save', {
            startNow,
            data: {
                title: form.title.trim(),
                section_id: form.section_id,
                min_duration_min: dur,
                max_duration_min: dur,
                priority: form.priority,
                is_one_off: form.is_one_off,
                recurrence_min_days: form.is_one_off ? 3650 : form.recurrence_min_days,
                recurrence_max_days: form.is_one_off ? 3650 : form.recurrence_max_days,
                pinned_dates: form.pinned_dates,
                task_type: 'filler',
            },
        });
    }
...
{#if open}
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60" on:click|self={() => dispatch('close')}>
        <div class="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-md">
            <div class="flex items-center justify-between px-5 py-4 border-b border-slate-700">
                <h2 class="font-semibold text-slate-100">Create Urgent Task</h2>
            ...
            <div class="px-5 py-4 space-y-4">
                <div>
                    <label class="block text-xs text-slate-400 mb-1">What needs doing? *</label>
                    <input
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="block text-xs text-slate-400 mb-1">Est. duration (min)</label>
                        <input
                            type="number"
                ...
                <div>
                    ...
                    <div class="flex gap-1">
                        <button
                            on:click={() => form.is_one_off = true}
                            class="flex-1 py-1.5 text-xs rounded font-medium transition-colors {form.is_one_off ? 'bg-indigo-700 text-white' : 'bg-slate-700 text-slate-400 hover:bg-slate-600'}">
                            One-off
                        </button>
                        <button
                            on:click={() => form.is_one_off = false}
                            class="flex-1 py-1.5 text-xs rounded font-medium transition-colors {!form.is_one_off ? 'bg-indigo-700 text-white' : 'bg-slate-700 text-slate-400 hover:bg-slate-600'}">
                            Recurring
                        </button>
                    </div>
            ...
            <div class="flex gap-2 px-5 py-4 border-t border-slate-700">
                <button on:click={() => dispatch('close')} class="px-4 py-2 text-sm bg-slate-700 hover:bg-slate-600 text-slate-200 rounded transition-colors">Cancel</button>
                <button
                    on:click={() => submit(false)}
                    disabled={!form.title.trim()}
                    class="flex-1 py-2 text-sm bg-slate-600 hover:bg-slate-500 disabled:opacity-40 text-white rounded font-semibold transition-colors">
                    Schedule
                </button>
                <button
                    on:click={() => submit(true)}
                    disabled={!form.title.trim()}
                    class="flex-1 py-2 text-sm bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white rounded font-semibold transition-colors">
                    Start Now
                </button>
            </div>
        </div>
